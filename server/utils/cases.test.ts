...
import { caseAssignedTo, accommodationCell, casesTableCaption, casesToRows, personCell } from './cases'
    ...
    it('returns formatted rows for a given list of cases', () => {
      ...
      expect(casesToRows(cases)).toEqual([
        [
          ...
          { html: accommodationCell('current', cases[0].currentAccommodation as AccommodationDto) },
          { html: accommodationCell('next', cases[0].nextAccommodation as AccommodationDto) },
          { html: '' },
        ],
      ])
    })
