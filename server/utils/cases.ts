...
export const casesToRows = (cases: Case[]): TableRow[] =>
  cases.map(c => [
    ...
    htmlContent(accommodationCell('current', c.currentAccommodation as AccommodationDto)),
    htmlContent(accommodationCell('next', c.nextAccommodation as AccommodationDto)),
    htmlContent(),
  ])
