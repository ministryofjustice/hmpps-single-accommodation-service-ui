...
import apiRoot from '../mockApis/apiRoot'
...
test.describe('Health', () => {
  ...
  test.describe('All healthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), apiRoot.stubPing(), tokenVerification.stubPing()])
    })
    ...
    test('Info is accessible', async ({ page }) => {
      ...
      const payload = await response.json()
      expect(payload.build.name).toBe('hmpps-single-accommodation-service-ui')
    })
  })

  test.describe('Some unhealthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), apiRoot.stubPing(), tokenVerification.stubPing(500)])
    })
